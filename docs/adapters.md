...
2. 适配器：`desktop/src/site-runtime/adapters-intl.js` / `adapters-intl2.js`（国际）或 `adapters-cn.js` / `adapters-cn2.js`（国内）。**分卷只因 300 行上限而存在，按站切，不按职责切**：当前 `intl` = Claude + Gemini、`intl2` = ChatGPT、`cn` = DeepSeek + 豆包 + 千问、`cn2` = Kimi + 元宝 + 智谱。
...
5. 测试：
   ...
   - 站点实现了 `submit` / `inject` / `attach` 时，才另加 `desktop/scripts/site-send-runtime.test.js` 用例。该文件只用 `vm` 执行 `adapters-cn.js` / `adapters-cn2.js`，针对 DeepSeek / 豆包 / Kimi / 元宝 验注入、发送与附件语义，**不是站点登记表**，加站点不改它不会红。**档位切换的回归不放这里**：国际站在 `desktop/scripts/intl-runtime.test.js`（Claude/Gemini）与 `intl2-runtime.test.js`（ChatGPT 滑块），国内站在 `cn-tier-runtime.test.js`（智谱/元宝/Kimi），模型正则另有 `claude-model.test.js` 与 `qwen-adapter.test.js` 两份专项，各份互不重叠。

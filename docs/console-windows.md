...
| `store.js` | 同步数据的本地 IndexedDB 层 |
| `data.js` | 本地记录与设备状态协议 |
| `sync.js` | Drive 同步串行状态机。300 行 |
| `sync-model.js` | 同步数据模型与版本比较（`SCHEMA=1`） |
...
| `synthesis-model.js` | 辅助综合载荷的纯拼装与校验 |
...
**`console/manage.js` 已删除**（原模板命名/删除二次确认，早已无 HTML 加载它）：模板命名与删除确认在 `compose.js`，分组的在 `scope.js`；`scripts/test-sync-ui.js` 曾对这个死文件 `readFileSync` 断言（永远绿），现已改断 `console/compose.js`。旧审查文档里的 `manage.js:NN` 行号只是历史记录。

## 窗口登记与生命周期

- **`amsWindows`（`storage.session`）host→`{id,owned}`**：`owned=true` 仅 `windows.create(popup)` 新建（`closeAll` 可自动关）；复用/收编的窗口 `owned=false`（不擅自关）。后续按 host 的操作**一律认登记表，不再裸查 tabs**——否则会误抓用户事后在主窗口开的同站标签。
- `managedTabForHost` 除了校验 `type:"popup"`，还要求活动标签停在目标 host；用户把受管窗导航走后绑定立即失效，调用方新建正确窗口。
- **窗口 id 跨浏览器重启失效**：重启后 id 重排，陈旧登记可能撞上无关 popup（如 OAuth 弹窗）被误关/误收编；按 id 只验 type 挡不住 popup 撞 popup。故 `onStartup` 一律清空 `amsWindows` / `amsConsoleWin` / `amsComposeWin` / `amsScopeWin` / `amsArchiveWin`。注意 `background.js` 的 `onStartup` 是用 `chrome.storage.local.remove` 删 `amsWindows`，而它实际在 `storage.session`——session 本来就随重启清空，所以无害，**别据此以为 `amsWindows` 在 local**——真值位置见本文「数据位置」。
- compose/scope/archive 三个伴侣窗**绝不进 `amsWindows`**，用各自专属 id 登记，随工作区联动、随 `closeAll` 一起关。开窗函数都有 in-flight 去重（`_openingCompose` 等）——背靠背两条消息会双开并孤儿化前一个。

## 平铺（`openTile`）

- **基准工作区取 `consoleWorkArea()`**（console 中心点所在显示器 → 拖到哪屏铺哪屏），同时根治副屏 reserve 混坐标系（console 在副屏时 `(c.top+c.height)-wa.top` 曾被算成跨屏距离）。
...
- 网格：`n≤4` 单排水平等分（各占满高度）；`n≥5` 用 `cols=ceil(sqrt(n))`、`rows=ceil(n/cols)`。可用高度有 **120px 下界**（console 被拖到屏幕底部时 reserve≈全高，防负高度建窗）。
- **`prune=false`（sendAll 隐式开窗）只为缺窗站开窗落格**：不关未勾选站、不重排已有窗口的用户手调布局（追问少数站时不得动别人正摆着答案的窗）。只有显式点「平铺」按钮才是全量重排语义；`prune=true` 时未勾选站 owned 的真正关闭、复用的仅解除登记。
...
- **群发取消（epoch）**：`_sendEpoch` / `cancelPendingSends()`。console 关闭或新一轮群发开始时，旧轮次未派发的站点立即以 `code:"cancelled"` 收场。长流程每个 `await` 后都要核对 epoch（硬约束）。
...
- **辅助综合的 URL 白名单**：`validSynthesisRequest` 只接受 `https:` 且 `new URL(site.url).hostname === site.host`，不合格返回 `invalid_request`（综合页已翻译为「请求无效」；防止把综合载荷发到任意 URL）。
...
**内部码，故意不进任何 ERR_KEYS、不给用户看**：`stale_run`（`background.js`）；`superseded`、`page_access_denied`、`page_empty`（`bg/page-context.js`）；`not_found`（`bg/data.js`、`bg/sync.js`、`bg/drive.js`、`bg/broadcast.js`）、`reconnect_required`、`local_write_failed`（`bg/data.js`、`bg/data-admin.js`）；`auth_failed`、`network_error`、`invalid_response`、`unauthorized`、`forbidden`、`rate_limited`、`server_error`、`request_failed`（`bg/drive.js`）；`sync_failed`（`bg/sync.js`）；`import_failed`、`invalid_header`、`newer_format`、`unknown_kind`（`bg/transfer.js`）；`invalid_record`（`bg/transfer.js` 与 `bg/archive-model.js` 共有）；`invalid_tags`、`invalid_source`、`invalid_synthesis`、`invalid_favorite`、`invalid_note`（`bg/archive-model.js`）。

## 超时预算表（改一处必须同步另一处）

| 场景 | 值 |
| --- | --- |
| 单站首轮 / 绝对截止（`submitWhenReady`） | 纯文本 22s / 44s；带图 45s / 90s。首轮到点只置 `retried` 标记继续等，绝对线才判 `timeout`。轮询 gap 800ms |
...
| console 圆点客户端兜底（`armDotTimeouts`） | 50s；带图 95s |
| send 按钮重新启用 | 48s；带图 95s。忙碌态通用 reset 兜底 30s |

`deadline` 透传与夹取规则见 `CLAUDE.md` 硬约束。**console 兜底必须严格大于后台预算**：小了圆点先翻红而后台还在发；后台加长了不改兜底，则 SW 被杀时圆点永久转圈。
...
- 细条内的交互一律做成内联控件（命名输入、二段删除确认）；需要纵向空间的功能开独立 popup：
  ...
  - **`scope.html`** —— 站点范围伴侣窗：**连续多选（勾一个不关窗、可以连着勾）**、分组管理、站点巡检；**失焦即关**，且不改变主 console 高度。这两条是一对矛盾约束：只记住「失焦即关」的人容易改成「选完即关」，那样多选就废了。
...
- **`aria-live` 播报区**：`console.html` 的 `#live`。群发进度、失败、汇总结果都要往这里写——96px 细条上的圆点变色对读屏用户不可见，这是唯一的进度通道。**无障碍属于不可简化项。**
...
- **`#privacy`** —— 本机数据（清空提问历史 / 清空结果库 / 重置本机数据，均带二段确认）+ 隐私说明 + 危险操作（删除云端同步数据，二次确认）。**改权限或数据行为时要同步这一区的文案。**

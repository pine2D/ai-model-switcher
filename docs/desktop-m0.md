...
- 紧凑模式下固定占高不超过 52px，依次容纳品牌标记、布局切换、提问框、档位、选择数量和发送/取消。
- 提问框默认单行；获得焦点或输入多行时，以浮层向下展开，不永久挤压视图，也不让 9 个站点在编辑过程中反复重排。
...
- 图标只替代跨平台含义稳定的布局、聚焦、重载和更多操作；档位与主发送动作保留文字，不用难以猜测的专有图标。

...
- 9 个站点页面保持独立会话；界面只挂载当前页的已选站点，用户能直接看到回答、思考动画、停止按钮和站点原生错误。
...
- 总览模式：只显示当前页的已选站点，并按 1–6 个站点动态排布。
- 聚焦模式：当前页显示一个主视图和最多 5 个实时次要视图；窄窗口自动采用此模式。
- 每页最多 6 个站点；选择 7–9 个站点时显示两组范围页签，换页只切换挂载关系，不销毁或重载页面。
...
- 应用菜单提供聚焦提问框、上一/下一站点和上一/下一组站点命令；分页快捷键为 `CmdOrCtrl+Shift+PageUp` / `CmdOrCtrl+Shift+PageDown`，避免焦点进入独立站点视图后无法返回 Shell。
...
- 单页超过 6 个完整网站会显著损害可读性，因此 7–9 个已选站点分为两页；后台页保留会话与运行状态，但不占用原生视图树和屏幕空间。

### 布局几何

- Grid：1 个站点铺满；2 个左右并排；3 个横向三分；4 个为 2×2；5 个为上 2 下 3；6 个为 3×2。
- Focus：1 个站点铺满；2 个采用约 2:1 主次分栏；3 个采用左侧主站加右侧两站；4–6 个使用 3×3 骨架，主站占左上 2×2，其余站点占右侧和底部。
- 请求 Grid 但任一格宽度 `<380 CSS px` 或总高度 `<210 CSS px` 时，自动采用 Focus；阈值按当前页实际站点数计算。
- Grid 永远恢复固定产品顺序。Focus 记住每页最近主站；页签切换后先恢复该页主站，否则取该页首站。
- 布局或页签切换只改 bounds、挂载关系和页面缩放，不销毁、不重载站点，不中断回答生成、滚动位置或登录会话。
...
- 回调页只说明“已收到授权，正在验证”，不提前声称 Drive 已连接；应用只有在首次 Drive 访问验证成功后才写入“已连接”状态，授权或验证超时仍保持未连接。
...
- 在 2048×1152、100% 系统缩放下，Shell 固定占高不超过 56px；1–6 个站点的 Grid/Focus 均保持网页可读。
- 在 1280×720、1440×900、1920×1080、2048×1152、2560×1440，以及 100%/125%/150% 系统缩放下，0–9 个任意选择的当前页 placements 均为正尺寸、互不重叠且不超出内容区。
- Grid/Focus、页签和 Focus 换站时 9 个 `webContents.id` 保持不变；原生视图树只包含当前页最多 6 个站点。
...
已完成 M0 纵向切片和动态选站布局：Electron 43 + Forge 7 + TypeScript + React 脚手架、单 `BrowserWindow`、9 个持久化 `WebContentsView`、每页最多 6 站的 Grid/Focus、安全导航和权限策略、隔离 preload、现有适配器加载、绝对 deadline/epoch 群发、页面与群发状态分层、三语错误/警示状态、单站重载和跨视图键盘焦点命令。界面只挂载当前页的已选站点；1–6 站动态排布，7–9 站分页且保留每页聚焦记忆，页签汇总后台页状态。群发结果区分失败与取消，并可按原发送范围重试；任意数量的已选站点使用同一套结果和重试逻辑。站点范围、档位和用户分组已持久化；命令栏支持最多 4 张 PNG/JPEG 图片的选择、粘贴、预览和兼容范围校验。回答可并行采集并定格到单窗口结果库；结果库临时 detach 而不销毁九个站点视图，支持搜索、收藏、标签、备注、最佳答案和 Markdown 预览/导出。辅助综合同样复用单窗口结果库，并只允许当前已勾选的站点作为综合目标。Google Drive 同步已迁移 schema 1 数据、原生 OAuth/PKCE、操作系统令牌保护、增量同步、退避、网络截止时间、未来格式只读和受保护的云端清理；未连接时后台同步不再发起授权，首次 Drive 验证成功后才标记已连接。Gemini 完成 Google 验证并返回后会执行一次受控重载。取消会锁定当前群发直至请求结算，并只重建仍在执行的对应站点视图。

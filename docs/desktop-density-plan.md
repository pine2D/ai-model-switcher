...
状态：已于 2026-08-24 完成。下方保留实际执行顺序和验收命令，复选框不再表示当前待办；持续验证结论见 `docs/desktop-m0.md`。
...
- [x] **Step 1: 写出 Grid 高宽门槛、宽/窄 Focus 几何和稳定交换的失败测试**
...
- [x] **Step 2: 运行布局测试并确认失败原因来自尚未实现的新签名和马赛克**
...
- [x] **Step 4: 让 ViewManager 持有 Focus 槽位顺序**
...
- [x] **Step 1: 写出显示偏好解析、密度尺寸和缩放决策的失败测试**
...
- [x] **Step 4: 实现 Shell 偏好持久化 helper**
...
- [x] **Step 5: 将显示偏好接入 ViewManager 和站点 zoom**
...
- [x] **Step 6: 增加经过 shell 顶层 sender 校验的 IPC 与原生菜单项**
...
- [x] **Step 7: 在 Renderer 启动时读取、应用和保存偏好**
...
- [x] **Step 1: 写出 52px Shell、24px 标题条和临时展开行为的失败契约**
...
- [x] **Step 2: 运行 Shell contract 并确认失败**
...
- [x] **Step 5: 将 Shell 改成单行 command deck**
...
- [x] **Step 7: 运行 Shell contract、桌面全量测试和类型检查**

Run: `cd desktop && npm test && npm run typecheck`

Expected: 全部 PASS，`index.tsx` 低于 180 行，新 TS/TSX 文件各自低于 300 行。

- [x] **Step 8: 提交紧凑 Shell 交付物**

...
function syncRuntime({ files = [], changes = [], downloads = {}, failHistory = false, goneOnce = false } = {}) {
  ...
  const store = {
    ...
    putFile: async (file) => { fileIndex.delete(`@${file.fileId}`); fileIndex.set(file.logicalKey, file); }, findFile: async (key) => fileIndex.get(key), getFile: async (id) => [...fileIndex.values()].find((file) => file.fileId === id), markFile: async (id, seenAt) => { const current = [...fileIndex.values()].find((file) => file.fileId === id); if (current) current.seenAt = seenAt; else fileIndex.set(`@${id}`, { fileId: id, seenAt }); },

...
- 回答整理：将各站最新回答汇总为 Markdown 后复制，也可保存归档或导出 `.md` 文件。
- 数据同步：通过 Google Drive 同步设置、模板、分组、提问历史和结果归档，并支持迁移包导入与导出。
...
在扩展设置页连接 Google Drive 后，可同步设置、模板、分组、提问历史和结果归档。未发送的草稿和窗口布局不会同步。此功能不使用 Chrome Sync。

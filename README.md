...
- 数据同步：通过 Google Drive 同步设置、模板、分组、提问历史和结果库，并支持迁移包导入与导出。
...
在扩展设置页连接 Google Drive 后，可同步设置、模板、分组、提问历史和结果库。未发送的草稿和窗口布局不会同步。此功能不使用 Chrome Sync。

- 扩展只申请 Google Drive 应用专属目录（`drive.appdata`）权限，不会读取或修改 Google Drive 中的其他文件。
- 同步到 Google Drive 的所有数据均为明文，包括提问历史、AI 回答，以及结果库中的收藏、标签、备注和最佳答案标记；迁移包也包含这些明文信息。PolyAsk 不提供端到端加密。

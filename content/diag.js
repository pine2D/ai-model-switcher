...
(function () {
  ...
  function common(a) {
    const checks = [{ name: t("diag_composer"), ok: !!S.findComposer() }];
    if (a.sendSel) checks.push({ name: t("diag_sendKey"), ok: !!document.querySelector(a.sendSel) });
    return checks;
  }
  Object.keys(S.adapters).forEach(function (key) {
    ...
    a.diagnose = function () {
      ...
      if (orig) { try { rest = orig(); } catch (e) { rest = [{ name: t("cs_diagError"), ok: false }]; } }
      // 缺 diagnose 的站保住「档位可读」兜底——core.js 的同款回退分支已被本包装遮蔽（包装后 a.diagnose 恒存在）
      else rest = [{ name: t("diag_tierReadable"), ok: S.getState() != null }];

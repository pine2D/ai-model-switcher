...
<!-- 最后与代码核对：2026-09-05 · desktop/package.json v0.25.1（下一版 1.0.0）。发版前重跑核对并更新这行。
...
- **`deadline` 是绝对时间戳、全链路透传**：`shell-ipc.ts:183`（44s / 带图 90s）→ `broadcast.ts` → `site-command-channel.ts` → `preload/site.ts` → site-runtime `submitPrompt` → `adapter.submit`/`attach` → `confirmSubmitted`。**循环等待、以及 ≥1s 的固定等待，一律夹取**：`Math.min(x, Math.max(0, deadline - Date.now()))`。唯一例外是「让渲染跟上」的毫秒级 sleep，单条 ≤500ms、不得叠成秒级。**已知偏离**：think/fast 切档路径尚未接 deadline（`core.js` 的 `await a[action]()` 零参调用，Kimi 单次 think 最坏约 12.6s），见 `docs/adapters.md` 待办。
